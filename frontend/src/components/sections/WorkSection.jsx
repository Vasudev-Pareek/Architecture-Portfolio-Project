...
const  WorkSection = ({ showCTA = true, limit = null, IsLimit = false }) => {
  ...
  useEffect(() => {
    const fetchProjects = async () => {
      try {
        ...
        // const res = await axios.get("https://architecture-portfolio-project-backend.onrender.com/api/projects?category=${category}");
        console.log("cateogry:", category)
        const res = await axios.get(`https://architecture-portfolio-project-backend.onrender.com/api/projects/all?category=${category}`);
  ...
  const displayedProjects = IsLimit ? projects.slice(0, limit) : projects;
